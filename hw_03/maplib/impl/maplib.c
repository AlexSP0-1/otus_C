...
typedef bool (*KeyCompareFunc)(MapKey, MapKey); //return true of keys are equal
...
struct Map {
    ...
    size_t count; //количество элементов, помеченных как удаленные
    size_t count_limit; //по первышению, выполняется рехеширование
    MapEntry *removed_entry; //используем для пометки удаленных ключей
};

Map *map_new_impl(HashFunc hash_func, KeyCompareFunc cmp_func, CreateEntryFunc create_func, FreeEntryFunc free_func)
{
    ...
    assert(cmp_func && "cmp_func must not be null");
    assert(create_func);
    assert(free_func);
